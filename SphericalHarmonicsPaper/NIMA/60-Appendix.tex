...
is taken into account, the early light sample is defined as PEs with $\Delta T=T^{PE}_{measured} - 
T^{PE}_{predicted}<$1~ns, where $T^{PE}_{measured}$ is measured arrival time and $T^{PE}_{predicted}$
...
R$<$3~m. In this case, the early light sample is defined as PEs with $\Delta T=T^{PE}_{measured} -
T^{PE}_{predicted}<$1~ns. Distributions of $\Delta T=T^{PE}_{measured} - T^{PE}_{predicted}$ for 
...
vertex reconstruction. The effect of primary vertex reconstruction resolution is implemented as smearing of 
the actual simulated vertex position, $R_{vtx}$, with the gaussian distribution
