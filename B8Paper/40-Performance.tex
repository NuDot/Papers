...
position. 
%For such events, a time cut of 33.5~ns on the PE arrival
%time can be applied to obtain an `early PE' sample that contains a
%high fraction of Cherenkov PEs. The default QE of 12\% for Cherenkov
%light and 23\% for scintillation light and 100\% photo-coverage are used
%in the simulation.
...
%a photon track length is
%mis-reconstructed due to imprecise reconstruction of 
...
\begin{figure*}[h]
  ...
  \caption{ The efficiency for 0\nbb-decay signal (\emph{red left-hand-scale}) and the
rejection factor for \B~neutrino background (\emph{blue right-hand-scale}) versus
$S_{01}$ for the default model (\emph{left panel}) and a
liquid scintillator with a 5-nsec risetime (\emph{right panel}).}
